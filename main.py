...
async def process_quiz_chain(email: str, secret: str, initial_url: str):
    ...
    while current_url and attempt_count < max_attempts:
        ...
        if (datetime.now() - attempt_start) > timedelta(minutes=3):
            print(f"Exceeded 3-minute limit for quiz at {current_url}")
            break
        
        print(f"Processing quiz at {current_url} (attempt {attempt_count})")
        ...
        try:
            ...
            print(f"Quiz instruction: {quiz_data['instruction'][:100]}...")
            
            # Get submit URL from page
            submit_url = extract_submit_url(html)
            if not submit_url:
                print(f"Could not extract submit URL from {current_url}")
                break
            
            # Solve quiz using LLM
            solution = await solve_quiz(quiz_data)
            print(f"LLM solution: {solution[:100]}...")
            ...
            if result.get("correct"):
                ...
                if next_url:
                    ...
                    print(f"No next URL provided. Reason: {result.get('reason')}")
                    break
...
async def handle_quiz(request: QuizRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(process_quiz_chain, request.email, request.secret, request.url)
